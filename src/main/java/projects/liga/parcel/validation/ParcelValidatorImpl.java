...
public class ParcelValidatorImpl implements ParcelValidator {
    ...
    public boolean isValid(List<String> parcel) {
        ...
        try {
            ...
            if (parcelFillingsAllowed.get(typeCode) == null) return false;

            if (parcelFillingsAllowed.get(typeCode).size() != parcel.size()) return false;

            for (int i = 0; i < parcelFillingsAllowed.get(typeCode).size(); i++) {

                if (parcelFillingsAllowed.get(typeCode).get(i).length()
                        != parcel.get(i).replace(" ", "").length()) return false;

                for (int j = 0; j < parcelFillingsAllowed.get(typeCode).get(i).length(); j++) {
                    if (parcelFillingsAllowed.get(typeCode).get(i).charAt(j)
