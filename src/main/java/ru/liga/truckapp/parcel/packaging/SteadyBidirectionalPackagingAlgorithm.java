...
public class SteadyBidirectionalPackagingAlgorithm implements ParcelPackager {
    ...
    /**
     * Функция, равномерно упаковывающая посылки в грузовики.
     * <p>
     * Все грузовики создаются с самого начала. Их число считаетмя равным минимуму из truckQuantity и количества посылок
     * (пустых грузовиков не будет).
     * <p>
     * Каждая итерация упаковки делится на 2 фазы. Во время первой фазы в каждый грузовик последовательно выкладывается максимально доступная посылка и
     * вычисляется разность объемов ее и максимальной посылки на итерации. Во время второй фазы алгоритм идет
     * по грузовикам в противоположном направлении и каждый раз пытается положить максимальную по объему доступную посылку,
     * объем которой не превышает текущей разности объемов максимальной посылки и объемов посылок, уже положенных
     * в заданный грузовик во время текущей итерации. Вторая фаза многопроходна. На следующей итерации направление укладки
     * во время каждой из фаз поменяется. Сделано это для достижения максимальной равномерности.
     * <p>
     * Направление прохода по грузовикам:
     * <p>
     * 1 итерация: -> (1 фаза) <- <- <- ... (2 фаза)
     * <p>
     * 2 итерация: <- (1 фаза) -> -> -> ... (2 фаза)
     * <p>
     * и т.д.
     *
     * @throws PackagingException исключение бросается, если при работе алгоритма не удается загрузить посылку в грузовик
     *
     * @param truckWidth ширина грузовика
     * @param truckHeight высота грузовика
     * @param truckQuantity количество грузовиков (максимальное, если посылок не меньше, чем грузовиков)
